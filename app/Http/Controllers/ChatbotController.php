   ...
   public function send(Request $request)
{
    ...
    try {
        $queryResult = $this->dialogflow->detectIntent($session->id, $userMessage);

        // Get the response text
        if ($queryResult && $queryResult->getFulfillmentText()) {
            $reply = $queryResult->getFulfillmentText();
        }

        // Get the detected intent category
        if ($queryResult && $queryResult->getIntent() && $queryResult->getIntent()->getDisplayName()) {
            $category = $queryResult->getIntent()->getDisplayName();
        }
    } catch (\Exception $e) {
